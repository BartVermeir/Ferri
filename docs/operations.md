...
|---|---|---|---|
| NAS IP | 2049 | TCP/UDP | NFS — file storage |
| `smtp.example.com` | 587 | TCP | SMTP — outbound mail |
...
| **Uploaded files** | NFS share | NFS server's own backup (NAS snapshots) |
...
nc -zv smtp.example.com 587
# Expected: Connection to smtp.example.com 587 port [tcp/submission] succeeded!

# If nc is not installed, use bash's built-in /dev/tcp (no extra tools needed):
timeout 5 bash -c 'cat < /dev/null > /dev/tcp/smtp.example.com/587' && echo "port reachable" || echo "port unreachable or timed out"
...
- Check NFS server health on the NAS
...
- [ ] NFS share is covered by the NAS snapshot or backup policy
