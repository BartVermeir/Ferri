...
**Primary deployment:** an example organisation  
...
**Alternatives considered:**
- **S3 multipart upload:** Requires S3-compatible storage. Example Org uses NFS/ZFS. Introducing an S3 gateway (MinIO) adds complexity and a moving part without benefit.
...
- The NAS is accessed via NFS and will be mounted into the container at deploy time.
...
**URL structure (compatible with Example Org existing structure):**
- Download: `/dl/<token>`
...
**Decision:** The codebase is published as open source (MIT or Apache 2.0 license, TBD). All configuration values are externalized. No hardcoded references to Example Org, their infrastructure, or their branding exist in the codebase.

**Rationale:**
- The software is generic by design. Example Org's deployment is one instance of a general-purpose tool.
...
**What Example Org-specific things live outside the repo:**
